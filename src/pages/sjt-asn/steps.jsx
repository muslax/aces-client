...
function Choices() {
  ...
  // We use progress.done + 1 because the keys are 1-based index
  const curSeq = progress ? progress.done + 1 : 0
  ...
  const BGREEN = "BGREEN cursor-pointer flex items-center justify-center border border-white w-10 h-10 rounded-full bg-gray-500 hover:bg-green-500"
  const BGREEN_GREEN = "BGREEN cursor-default flex items-center justify-center border border-white w-10 h-10 rounded-full bg-green-500"
  const BRED = "BRED cursor-pointer flex items-center justify-center border border-white w-10 h-10 rounded-full bg-gray-500 hover:bg-red-500"
  const BRED_RED = "BRED cursor-default flex items-center justify-center border border-white w-10 h-10 rounded-full bg-red-500"
  ...
  return (
    <div>
      {/* <div className="border-ts bg-white border-ts border-b bg-gradient-to-t from-white text-gray-800 px-4"> */}
      ...
      <div className="max-w-3xl mx-auto px-6s pt-12 mb-16">
      {options?.map(opt => (
        <div key={opt.elm} className="flex flex-row items-stretch items-center rounded">
          ...
          <div className="flex py-1 border-l border-gray-400">
            <div className="flex rounded-r-md bg-indigo-100 text-sm text-white font-bold px-2">
        ...
        <div className="mt-12 text-center">
          {most && least && (
            <button id="submit" type="submit" className={btnActive}>
      ...
      {/* <pre className="pre">{JSON.stringify(testItem, null, 2)}</pre> */}
    </div>
  )
}
